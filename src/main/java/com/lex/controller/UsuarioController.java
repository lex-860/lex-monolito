...
public class UsuarioController {
    ...
    @GetMapping("/{id}") // usuario/1
    public ResponseEntity<UsuarioDados> detalhar(@PathVariable(value = "id") Long id) {
        return new ResponseEntity<UsuarioDados>( service.buscar(id), HttpStatus.OK );
    }

    @GetMapping() //usuarios?username=rafa
    public ResponseEntity<UsuarioDados> detalhar(@RequestParam(value = "username") String username) {
        return new ResponseEntity<UsuarioDados>( service.buscar(username), HttpStatus.OK );
    }
    ...
    public ResponseEntity<String> carteira(@PathVariable(value = "id") Long idUsuario, @RequestBody CarteiraCadastro cadastro) {
        this.service.salvar(idUsuario, cadastro); // TODO refatorar para pegar usuario tb do spring e remover parametro
